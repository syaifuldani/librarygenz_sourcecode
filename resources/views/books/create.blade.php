<x-app-layout>
    ...
    <div class="mb-8">
        ...
        <h2 class="font-serif text-3xl font-bold tracking-tight text-navy-900">
            Daftarkan <span class="text-coral-500">Buku Baru</span>
        </h2>
        <p class="mt-2 text-sm text-navy-500">
            Isi formulir lengkap untuk menambahkan koleksi buku fisik baru ke dalam katalog perpustakaan.
        </p>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="Contoh: Bumi Manusia"
                    ...
                    <div>
                        ...
                        <input type="text" name="author" id="author" required value="{{ old('author') }}" placeholder="Contoh: Pramoedya Ananta Toer"
                            class="w-full px-3 py-2 text-xs bg-cream-100/50 border border-cream-300 rounded-lg text-navy-800 focus:outline-none focus:ring-1 focus:ring-coral-500 focus:border-coral-500 transition-colors">
                        @error('author')
                            <p class="text-xs text-coral-600 mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" id="stock" required min="0" value="{{ old('stock', 1) }}"
                            class="w-full px-3 py-2 text-xs bg-cream-100/50 border border-cream-300 rounded-lg text-navy-800 focus:outline-none focus:ring-1 focus:ring-coral-500 focus:border-coral-500 transition-colors">
                        @error('stock')
                            <p class="text-xs text-coral-600 mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" placeholder="Tuliskan sinopsis singkat mengenai buku ini..."
                        class="w-full px-3 py-2 text-xs bg-cream-100/50 border border-cream-300 rounded-lg text-navy-800 focus:outline-none focus:ring-1 focus:ring-coral-500 focus:border-coral-500 transition-colors">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-xs text-coral-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Cover Image Upload -->
                <div>
                    <label for="cover_image" class="block text-xs font-semibold text-navy-700 uppercase tracking-wider mb-2">Unggah Gambar Sampul</label>
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-xs text-navy-600">
                                <label for="cover_image" class="relative cursor-pointer rounded-md font-bold text-coral-500 hover:text-coral-600 focus-within:outline-none">
                                    <span>Unggah file sampul</span>
                                    <input id="cover_image" name="cover_image" type="file" class="sr-only">
                                </label>
                                <p class="pl-1">atau seret dan lepas gambar</p>
                            </div>
