<x-guest-layout>
    <div class="mb-6">
        <h3 class="font-serif text-2xl font-bold text-navy-900">Atur Ulang Password</h3>
        <p class="text-xs text-navy-500 mt-1">Silakan buat kata sandi baru untuk akun Anda.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.store') }}" x-data="{ loading: false }" @submit="loading = true">
        ...
        <div class="mt-6">
            <x-primary-button class="w-full py-3" ::disabled="loading">
                <span x-show="!loading">{{ __('Perbarui Password') }}</span>
